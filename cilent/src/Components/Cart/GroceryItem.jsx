...
export const GroceryItem = ({ _id, title, quantity, price, image, totalquantity }) => {
  ...
  const [updatedQuantity, setUpdatedQuantity] = useState(totalquantity);
  ...
  const handlePlus = () => {
    ...
    handleAddToCart(updatedQuantity + 1)
  }
  const handleMinus = () => {
    setUpdatedQuantity((prev) => prev + 1)
    handleAddToCart(updatedQuantity + 1)
  ...
  const handleRemoveCartItem = (itemId) => {
    ...
    dispatch(removeCartItem(username, itemId, updateToken));
  };
  ...
  return (
    <div className="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
      ...
      <div className="flex justify-center w-1/5">
        ...
        <input className="mx-2 border text-center w-8" type="text" value={updatedQuantity} readOnly />
        <FontAwesomeIcon icon={faPlus} className="fill-current cursor-pointer text-gray-600 w-3" onClick={handlePlus} />
      </div>
